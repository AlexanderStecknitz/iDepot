...
class DepotResourceKotlin(private val depotService: DepotServiceKotlin, private val depotMapper: DepotMapper) {

    @GetMapping
    fun findAll(): ResponseEntity<List<DepotDTOKotlin>> {
        val foundDepots: List<Depot> = depotService.findAllDepots();
        if (foundDepots.isEmpty()) {
            return ResponseEntity.notFound().build()
        }
        val foundDepotsDto: List<DepotDTOKotlin> = foundDepots.map { depotMapper.toDepotDTO(it) }
        return ResponseEntity.ok(foundDepotsDto)
    }

    @GetMapping(path = ["/{email}"])
    fun findAllByEmail(@PathVariable email: String): ResponseEntity<List<DepotDTOKotlin>> {
